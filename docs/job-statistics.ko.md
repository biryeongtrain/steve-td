...
요약 Dialog에는 선택 수, 선택률, 승률, 평균 순위와 평균 최종 라운드가 나옵니다. 공식·창작 빌더의 직업별 요약은 제목·열 헤더·상단 흰색 구분선으로 구성한 헤더, 직업 행 본문, 하단 흰색 구분선을 각각 별도 Dialog 본문으로 나눕니다. 직업 버튼을 누르면 R1~R10, R11~R20, R21~R30, R31~R40 통과율을 범위 헤더 없이 4개의 세로 열로 나눈 표와 특성 조합 통계를 확인할 수 있습니다. 상세 표의 내부 폭은 가장 긴 5열·6열 헤더가 다음 줄로 넘어가지 않도록 여유를 두고 정렬합니다. 표본·특성 조합·전투·인컴 소환·라인 방어 지원도 섹션 제목·열 헤더·흰색 구분선으로 구성한 헤더와 수치 본문을 별도 Dialog 본문으로 나누며, 그 사이에 한 줄 간격을 둡니다. 라운드 통과율은 섹션 제목과 본문을 나누되 범위 헤더와 내부 흰색 구분선은 표시하지 않습니다. 라운드 통과율을 제외한 표에는 세로 구분자를 표시하지 않으며, 특성 버전은 집계 키에는 유지하지만 화면 이름에는 표시하지 않습니다.
