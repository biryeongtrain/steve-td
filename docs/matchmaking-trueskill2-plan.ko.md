...
- 승패만 저장하면 너무 거칠고, 라운드/소환/처치/수입 같은 보조 지표를 추후에 rating 보정에 쓸 수 있다.
...
| `dynamicsFactor` | `0.083333` | 오래 플레이하지 않은 플레이어의 불확실성 회복 폭. |
...
- append-only로 기록한다. 경기 중 산출된 원본 이벤트와 경기 종료 요약을 추후에 재계산할 수 있어야 한다.
...
파일 형식은 JSONL을 우선한다. 한 경기의 이벤트가 많아질 수 있고, 경기 중 append가 쉬우며, 추후에 외부 분석 도구로 읽기 좋다.
...
- UI 재시도에서 reward line 재구성이 필요하면 추후에 `MatchResultRepository`와 reward snapshot 저장을 추가한다. 현재 prework는 중복 재적용 방지가 우선이다.
...
| placementCountByTeam | 신규 플레이어가 한 팀에 몰리는지 확인 |
...
추후에 자동 구현 루프를 돌릴 경우 launch-ready 초안은 다음과 같다.
