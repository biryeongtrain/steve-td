...
| 민들레 계열 id | `diamondPerSecond` | 살아 있는 동안 1초마다 라인 주인에게 지급하는 다이아입니다. 환경 틱 주기와 무관하게 정확히 1초 간격으로 돕니다. |
...
| 균사 계열 id | `explosionRadius`, `explosionDamageMultiplier` | 폭발 반경과 타워 피해 대비 배율입니다. 실제 피해는 `damage × 배율 × (1+개화)`입니다. |
...
| `plant_soil_mycelium` | `damageTakenBonus`, `debuffDurationTicks` | 균사 위의 적이 타워에게 받는 피해 증가와 지속 시간입니다. |
| `plant_soil_mycelium` | `poisonDamagePerStack`, `poisonMaxStacks`, `poisonDurationTicks`, `poisonTickIntervalTicks` | 포자 중독입니다. T2 이상 전투 타워만 겁니다. |
| `plant_soil_desert` | `moveSpeedReduction`, `debuffDurationTicks` | 모래 위의 적 이동 속도 감소와 지속 시간입니다. |
| `plant_soil_desert` | `thornReflectRatio` | 받은 피해를 가시로 반사하는 비율입니다. T2 이상 전투 타워만 반사합니다. |
| `plant_soil_podzol` | `rangeBonus`, `attackSpeedBonus` | 회백토 위 전투 타워의 사거리와 공격 속도 증가입니다. |
