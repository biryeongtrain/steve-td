...
> 2026-09-24 테스트 결정: 리롤 한 번으로 세 장을 모두 다시 뽑는다. 경기 전체 합계 5회이며 R15·R25와 재접속 때 충전하지 않는다. 헤더와 리롤 화면에 남은 횟수를 표시한다. 로컬 구현이며 운영 반영은 별도다.

> 기준일: 2026-09-06 · 기준 브랜치: `s3-release` · 적용 모드: `NORMAL`. 자동 선택과 설정 알림 제거는 로컬 반영 상태이며, 실제 클라이언트 검증과 라이브 적용은 별도다.

2026-09-06 추가 결정: 선택 60초 마감 시 유효 후보 중 한 장을 무작위 지급하며, 모두 부적격이면 같은 희귀도의 즉시 다이아를 지급한다. 실패한 확정은 마감 전 다시 시도할 수 있다. 증강의 대상·모드 설정은 본인 화면에만 표시하고 매 라운드 전체·팀 채팅으로 알리지 않는다. 최초 획득 시 카드명·희귀도 알림은 유지한다. 이 결정이 아래 설계 이력의 `SKIPPED · TIMEOUT` 및 설정 공개 규칙보다 우선한다. 명시적 건너뛰기와 과거 시간 초과 기록은 보존한다.
...
| 시간 초과 | 한 장을 무작위 지급하고 `SELECTED`로 기록. GUI 이력은 `AUTO_SELECTED · TIMEOUT` |
| 다시 열기 | `/증강` 또는 `/semiontd augment ui` |
| 준비 시간 | 증강 선택 60초를 기존 준비 25초 앞에 추가해 총 85초. 추가 60초 동안만 전원의 에메랄드 자동 생산 중단. 그 밖의 준비 단계는 기존 25초 |
...
일반 후보 35장과 후보 부족용 예비 보상 9장을 구현했다. 번들 기본값의 공개 풀은 꺼져 있으며, 2026-09-06 승인된 라이브 설정에서는 켰다. 이후 변경의 자동 테스트 통과는 추가 배포 승인을 뜻하지 않는다.
...
기본 준비 시간은 `SemionGame.DEFAULT_PREPARE_TICKS`의 25초다. `startPreparePhase`는 살아 있는 플레이어의 미확정 제안을 하나 이상 만들면 그 앞에 선택 시간 60초를 추가한다. `currentPrepareDurationTicks`는 총 85초이며, `tickPrepare`와 `remainingPrepareSeconds`가 함께 사용한다. `augmentSelectionDurationTicks`로 추가 시간의 종료를 구분하며 별도 `AUGMENT_PHASE`는 만들지 않는다. `EconomyService.tickEmerald`는 이 추가 시간 동안 자동 생산을 지급하지 않는다. 일반 준비가 시작되면 생산을 재개하고, 중단한 생산량을 나중에 보충하지 않는다.
...
| 층 | GUI에 적용하는 내용 |
|---|---|
| Mechanics | 세 장 제안, 같은 희귀도, 전체 리롤 경기당 5회, 시간 제한, 대상·모드 선택 |
| Dynamics | 현재 보드 비교, 제안 유지 또는 전체 교체, 다음 웨이브 대비, 상대 선택 정찰 |
| Aesthetics | 도전, 발견, 자기표현, 프리즘 공개의 기대감, 확정 순간의 해소 |

초기안은 TFT의 카드 칸별 리롤을 참고했다. 2026-09-24 테스트에서는 다양한 후보를 확인할 수 있도록 전체 리롤 5회로 바꾼다. 플레이어는 현재 제안을 포기할지, 남은 횟수를 R15·R25에 쓸지 결정한다.
...
| 규칙 | 세 장 중 하나, 경기 전체 리롤 5회, 준비 종료 전 확정 |
...
```mermaid
flowchart TD
    A["경기 시작: 희귀도 순서 저장, 비공개"] --> B["제안 준비 단계: 카드와 마감 틱 저장"]
    B --> C["희귀도 연출 20틱"]
    C --> D["카드 3장 비교"]
    D -->|"단순 카드"| F["카드 초안"]
    D -->|"대상·모드·손실 있음"| F
    ...
    G --> Q
    Q --> E["서버 재검사 후 확정"]
    D -->|"전체 리롤"| H["세 장 재생성, 1회 차감, 제안 버전 증가"]
    H --> D
    D -->|"닫기"| I["제안 유지"]
    I -->|"/증강"| D
    D -->|"건너뛰기"| J["건너뛰기 확인"]
...
- 생존 플레이어의 제안 카드 세 장과 `offerRevision`을 만든 뒤 선택 시간 60초와 일반 준비 25초를 차례로 진행한다. 준비 시작 틱을 `prepareStartTick`이라 하면 `inputAllowedTick=prepareStartTick+20`, `deadlineTickExclusive=prepareStartTick+60*20`이다. 제안 변경 요청은 `currentTick >= inputAllowedTick && currentTick < deadlineTickExclusive`일 때만 받는다.
- 추가 60초 동안 전원의 에메랄드 자동 생산을 멈춘다. 일찍 선택·건너뛰기·재접속을 해도 공통 종료 시점은 바뀌지 않는다. 생산 보너스와 R25 생산 배율도 중단 대상이다. 배치·구매와 별도 보상·환불 규칙은 유지하며, 이후 일반 준비 25초 동안 자동 생산을 재개한다.
...
중도 참여자의 예약된 R5 제안은 다음 준비 단계가 시작될 때 생성한다. 희귀도 연출 20틱 뒤 주 화면을 열며, 해당 라운드에도 전원이 선택 60초와 일반 준비 25초를 함께 사용한다.
...
2026-09-22 결정: 경기 시작에는 희귀도 순서를 알리지 않는다. 해당 선택지가 열릴 때만 희귀도를 공개한다. 선택 기록에서 이후 라운드의 희귀도는 `미공개`로 표시한다.
...
```text
R15 골드 증강
마감: 선택 시간 60초 종료 · 이후 일반 준비 25초 · 전체 리롤 5/5회 남음
...
- `리롤`은 전체 교체 화면을 연다. 새 제안을 생성할 때만 해당 플레이어의 경기 잔여 횟수에서 1회를 소비한다.
...
`리롤`을 누르면 교체 범위와 남은 횟수를 보여 준다.

```text
같은 등급의 선택지 세 장을 모두 다시 뽑습니다.
경기 전체 리롤: 5/5회 남음
...
전체 리롤 버튼을 누르면 다음 순서로 처리한다.

1. 서버가 리롤 잔여 횟수, 단계, 마감과 `offerRevision`을 확인한다.
2. 이번 마일스톤에서 이미 본 일반 카드, 현재 부적격 카드와 보유 카드에 충돌하는 카드를 제외한다.
3. 최초 제안과 같은 방식으로 세 장을 구성한다. ID·계보 중복, `SAFE` 보장과 대가형 상한을 검사한다. 후보가 부족하면 예비 보상을 다시 넣을 수 있다.
4. 새 카드가 한 장도 없으면 제안과 횟수를 유지한다. 같은 시드와 입력은 같은 제안을 만든다.
5. 성공하면 1회를 차감하고 `offerRevision`을 올리며 초안을 지운다. 마감 시간은 유지한다.
6. 리롤을 사용한 새 화면을 연다.

횟수를 모두 썼거나 새 후보가 없으면 실행 버튼을 숨기고 이유를 본문에 적는다. 일반 `SAFE` 후보가 없으면 즉시 다이아를 포함한다.

`세 장 모두 리롤 · 1회 소비` 버튼을 누르면 추가 확인 없이 처리한다.
...
| 리롤 | 플레이어별 경기 잔여 횟수, 마일스톤별 이미 본 카드 ID, 교체 전후 세 장 |
...
/semiontd augment reroll <offerRevision> <requestId>
...
| 리롤 | 교체 전후 세 장, 성공·실패 이유. 단일 카드 칸은 기록하지 않음 |
...
2. 세 장 전체 리롤과 경기 합계 5회 한도, 제안·설정 초안, 카드별 1회 설정, 확정, 건너뛰기와 만료를 단위 테스트한다.
