...
`cosmetics.json`, `profiles.json`, `build_guides.json`, `semiontd.db`, `job-statistics.db`는 운영 데이터입니다. 치장 상품은 주 손 아이템을 든 뒤 `/semiontd cosmetic add <id> <price>`로 등록합니다. 운영 데이터는 직접 수정하기보다 명령어와 서버 백업으로 관리합니다.
